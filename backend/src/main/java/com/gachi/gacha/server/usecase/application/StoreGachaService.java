...
public class StoreGachaService {
    ...
    @Transactional
    public StoreGachaInfo addStoreGacha(StoreGachaCreatCommand command) {
    ...
    public Page<GachaSummaryInfo> findGachasByStoreId(Long storeId, Pageable pageable) {
        Page<Gacha> gachas = storeGachaJpaRepository.findGachasByStoreId(storeId, pageable);
        return gachas.map(GachaSummaryInfo::from);
    }

    @Transactional
    public StoreGachaInfo removeStoreGacha(StoreGachaDeleteCommand storeGachaDeleteCommand) {
